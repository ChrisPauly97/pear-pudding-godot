...
func test_siege_persists_same_day() -> void:
	...
	assert_false(_sm.town_siege.get_active_siege().is_empty(), "siege must persist within the same day")

func test_siege_cleared_after_1_day_rollover() -> void:
	_sm.days_elapsed = 5
	_sm.town_siege.start_siege("madrian")   # day_started = 5
	_sm.increment_day()          # days_elapsed → 6; age = 6-5 = 1 >= 1 → timeout
	assert_true(_sm.town_siege.get_active_siege().is_empty(), "siege must clear after 1 day")
...
func test_siege_started_today_does_not_clear_same_rollover() -> void:
	...
	assert_false(_sm.town_siege.get_active_siege().is_empty())
