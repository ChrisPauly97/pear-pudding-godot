...
func _build_ui() -> void:
	...
	var title := _UiUtil.make_label("Achievements", int(_vh * 0.038), Color.WHITE, HORIZONTAL_ALIGNMENT_LEFT,
			header_row)
...
func _make_row(a: Dictionary, is_unlocked: bool, current: int) -> Control:
	...
	# Lock / check icon
	var icon := _UiUtil.make_label("[OK]" if is_unlocked else "[  ]", int(_vh * 0.022),
			Color(0.3, 1.0, 0.3) if is_unlocked else Color(0.5, 0.5, 0.5), HORIZONTAL_ALIGNMENT_LEFT, hbox)
	...
	if target > 1 and not is_unlocked:
		var prog_lbl := _UiUtil.make_label("%d / %d" % [mini(current, target), target], int(_vh * 0.022),
				Color(0.7, 0.85, 1.0), HORIZONTAL_ALIGNMENT_LEFT, text_vbox)

	# Reward indicator
	var reward_id: String = str(a.get("reward_card_id", ""))
	if reward_id != "":
		var reward_lbl := _UiUtil.make_label("Reward: Legendary card", int(_vh * 0.022),
				Color(1.0, 0.8, 0.2) if is_unlocked else Color(0.5, 0.4, 0.1), HORIZONTAL_ALIGNMENT_LEFT, text_vbox)
