...
func _ready() -> void:
	...
	# On portrait mobile vh >> vp.x; clamp to shorter dimension so elements aren't oversized.
...
func _make_slot_row(slot: int, ref: float, vp: Vector2) -> Control:
	...
	detail_lbl.add_theme_font_size_override("font_size", int(ref * 0.022))
