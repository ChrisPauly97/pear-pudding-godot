...
func _check_game_over() -> void:
	...
	if _state.is_game_over():
		...
		if w == 0:
			...
			if bool(enemy_data.get("is_boss", false)):
				_show_victory_overlay_boss(pool)
			else:
...
func _show_victory_overlay_boss(reward_cards: Array[String]) -> void:
	var overlay := PanelContainer.new()
	overlay.set_anchors_preset(Control.PRESET_FULL_RECT)
	var style := StyleBoxFlat.new()
	style.bg_color = Color(0.05, 0.05, 0.1, 0.92)
	...
	var btn := Button.new()
	btn.text = "Collect" if not reward_cards.is_empty() else "Continue"
	btn.custom_minimum_size = Vector2(_vh * 0.18, _vh * 0.06)
	btn.add_theme_font_size_override("font_size", int(_vh * 0.025))
	...
	btn.pressed.connect(func() -> void:
		overlay.queue_free()
		GameBus.battle_won.emit({"card_rewards": final_rewards})
	)
	vbox.add_child(btn)
