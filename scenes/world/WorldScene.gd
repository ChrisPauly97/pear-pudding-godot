...
## Creates the co-op feature modules and registers them as NetSync RPC targets.
## Idempotent: a PvP battle detaches and re-adds WorldScene without tearing the
## modules down, and _setup_coop can run again after a reconnect.
func _ensure_coop_modules() -> void:
	if coop_social == null or not is_instance_valid(coop_social):
		coop_social = _CoopSocial.new()
		coop_social.name = "CoopSocial"
		coop_social.set("_world", self)
		add_child(coop_social)
	if _net_sync != null:
		_net_sync.call("register_handler", coop_social)
