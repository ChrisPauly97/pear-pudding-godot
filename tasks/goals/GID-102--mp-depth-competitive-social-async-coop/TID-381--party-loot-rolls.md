...
- **`scenes/world/WorldScene.gd`**:
  ...
  - `_grant_chest_loot_to_token` grants cards (via `CardDropUtil` rarity/stat rolls +
    ...
    Equipment drops are explicitly out of scope (see BID-025).
...
- **`tasks/backlog/BID-025--no-session-scoped-equipment-inventory.md`** (new) — logged the
...
- `docs/agent/multiplayer-coop.md`:
  - Retitled "Loot rule — first-opener-takes" to "Loot rule — first-opener-takes (default),
    ...
    physical pickup path, and the equipment-drop scope cut (with a BID-025 pointer).
