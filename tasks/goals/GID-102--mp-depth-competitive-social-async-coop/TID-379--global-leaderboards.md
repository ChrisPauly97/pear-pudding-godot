...
- Version bump: `SessionState.CURRENT_SESSION_VERSION` 4 → 5, migration backfills
...
overwrites)/update(worse score is a no-op)/sort-desc/cap-at-20, round trip, migration v4→v5
...
- **`game_logic/net/SessionState.gd`** — added `leaderboards: Dictionary` field
  ...
  Bumped `CURRENT_SESSION_VERSION` 4 → 5 with a migration backfilling `leaderboards` when
...
- **`tests/unit/test_session_state.gd`** — added 15 new tests covering: leaderboards
  ...
  `get_pve_leaderboard` limit, round-trip, snapshot shape, migration v5 backfill +
  preserves-existing-value + garbage-field tolerance.
- **`tasks/backlog/BID-027--coop-clear-value-lacks-boss-tier-and-timing.md`** — new
...
**Version bump used:** `SessionState.CURRENT_SESSION_VERSION` 4 → **5**.
