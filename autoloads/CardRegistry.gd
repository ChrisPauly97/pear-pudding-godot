	...
	for preloaded in all:
		...
		if card_id != "":
			...
			if illus == null:
				...
				var illus_tex: Texture2D = _SpriteRegistry.card_illustration_texture(illus_key, magic_branch)
				if illus_tex == null:
					illus_tex = TextureGen.card_illustration(illus_key, magic_branch)
				res.set("illustration", illus_tex)
		else:
