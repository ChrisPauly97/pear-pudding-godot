...
- **Water clearance:** `ChunkRenderer._water_dry_points` collects wall/cracked/path tile centres in the snapshotted grid, the full footprint of a ruin (chunk has doors → bounding box of its wall tiles) and doors/landmarks/waystones/mana wells; the footprint/entity points are used only when ≥ the fade reach inside the chunk. `WaterMath.water_at = intensity × structure_fade` (0 within `DRY_RADIUS` 2.5, full beyond +`DRY_FADE` 2.5). The reach must stay ≤ 5 units (grid margin `TILE_CHECK` 3 tiles) or border vertices disagree between chunks and the water seams. Grass, props and `ChunkRenderer.water_at_world()` (footstep splashes) use the same value.
- **Biome ground & grass:** `BiomeDef.GROUND_DESAT` desaturates the green ground textures before tinting (desert sand, scorched ash, mountain snow; terrain `ground_desat`), `WALL_MOSS` turns wall moss off in desert/scorched (`wall_moss`), `GRASS_DENSITY` thins tufts (`keeps_grass`, applied in `ChunkRenderer.prepare_terrain`), and `GRASS_RECOLOR` recolours blades via the `grass_recolor` instance uniform on each chunk's grass MultiMeshInstances. Scorched `GRASS_TINT` lifted to 0.46/0.30/0.20 (the old near-black tint let bluish fog turn the ground navy).
