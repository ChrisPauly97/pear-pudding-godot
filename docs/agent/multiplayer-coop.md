...
are the read side. `CURRENT_SESSION_VERSION` bumped 4 → 5; the migration backfills
...
- **Co-op boss clears** (`GameBus.coop_pve_battle_ended(did_win)` →
  ...
  inventing new cross-battle plumbings. Logged as BID-027 for a future task to
...
| `tests/unit/test_session_state.gd` | unit (auto-run) | SessionState round-trip, member lookup/create by token, resume-without-reset, starter seeding (token-salted UIDs), migration scaffold + garbage tolerance; pvp stats fields + round-trip + migration v3 backfill; pvp_rating/pvp_games fields + round-trip + v4 backfill + derived `get_leaderboard` ordering/limit/record; party_bounties default/round-trip/garbage tolerance; `leaderboards` {spire, coop_clears} default + `record_pve_score` insert/own-better-overwrites/worse-and-equal-are-no-ops/sort-desc/cap-at-20/unknown-board-and-blank-token no-ops, `get_pve_leaderboard` limit, round-trip + snapshot shape, migration v5 backfill + preserves-existing + garbage-field tolerance (45 cases) (GID-095 / GID-101 / GID-102) |
