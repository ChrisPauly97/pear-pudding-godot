...
- `player_ambush = true` when caught at `IDLE` — covers both wanderers (never alerted at all, since they get no awareness `Area3D`) and tracking enemies reached before they noticed. `BattleScene.modifiers._apply_ambush_modifiers()` sets the *enemy* hero's `health`/`max_health` to `round(max_health * 0.8)` (floor 10) and shows `BattleResultUI.show_ambush_banner(true)` ("Ambush!", green).
...
**Engage cooldown (GID-069, corrected BID-058):** there is no per-enemy `EnemyNPC.engage_cooldown` field — that description predates the TID-427 async `engage()` rewrite and was never updated. The only protection against instant re-engagement after a flee or respawn is the global "Post-battle immunity" window above (`SceneManager._proximity_engage_blocked`, 2 s, see line 205): it blocks *all* proximity engagement for 2 s regardless of which enemy was fled from, rather than a per-enemy cooldown keyed to the specific `EnemyNPC`.
