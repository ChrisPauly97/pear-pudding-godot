...
- **Mount sprite** (`_mount_sprite: Sprite3D`): 48×24 px generated by `TextureGen.mount_horse()`, PIXEL_SIZE=0.05, positioned at y=0.6 (below the wizard sprite). Shown/hidden via `_update_mount_visuals()` which is called from `_on_mount_state_changed()` connected to `GameBus.mount_state_changed`.
...
Cached static method returning a 48×24 ImageTexture. Brown body + mane/tail + four legs, transparent background.
...
| `TextureGen` | `mount_horse()` procedural sprite texture |

## Asset Requirements

- No PNG assets required — mount sprite generated at runtime by `TextureGen.mount_horse()`.
- No new `.tres` or `.uid` files — mount data is a GDScript dictionary in `MountRegistry.gd`.
