...
| Arena layout | **Diagonal**: your token bottom-left, the enemy's top-right; each side's slots step top-left → bottom-right as two **parallel front lines** — the enemy's one card height (+8 % gap, `ROW_GAP`) above and 0.9 card widths right of yours, the pair centred in the arena (`RealtimeVisuals.row_origins`, unit-tested for no overlap at 16:9 and 20:9) (`DiagonalBoard.gd` attached at runtime to the board HBoxes, which are reparented out of the stacked areas); no divider; the hand sits at the bottom. Re-laid out on viewport resize (`RealtimeVisuals.apply_layout`) |
