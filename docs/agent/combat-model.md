...
| Arena layout | **Diagonal**: your token bottom-left, the enemy's top-right; each side's slots step top-left → bottom-right (`DiagonalBoard.gd` attached at runtime to the board HBoxes, which are reparented out of the stacked areas); no divider; the hand sits at the bottom. Re-laid out on viewport resize (`RealtimeVisuals.apply_layout`) |
