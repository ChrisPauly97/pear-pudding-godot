...
- **Impact animation layer (TID-426):** `BattleFx.animate_attack(panel, target_pos, speed_scale, hit_stop)` (async) lunges an attacker panel ~60% of the way toward its target and back (`TRANS_QUAD`, z_index raised during travel), with an optional hit-stop pause at the apex for damage ≥5 or lethal hits. `BattleFx.animate_death(panel, speed_scale)` (async) duplicates a dying panel into `_float_layer` as a ghost and shrinks/fades/rotates it, so `_refresh_all()` rebuilding the zone reads as a death beat instead of a pop. `_execute_attack()` awaits the lunge *before* mutating damage, then awaits `_animate_deaths_from_snapshot()` (BattleScene) — a diff between a pre-mutation `BattleFx.snapshot()` and the currently-alive card ids (`BattleFx.detect_deaths()`, pure) — before refreshing, so any death (player attack or AI kill) gets the beat via the same mechanism. `_execute_ai_actions()` reuses the same death-animation helper (covers regular and boss AI turns) but does not lunge — `BasicAI.decide_turn()`'s actions are closures that mutate `GameState` directly with no attacker/target panel surfaced to BattleScene. `_animate_card_travel()` ghost-tweens a played minion from its hand panel rect to its board slot center (`TRANS_BACK`/`EASE_OUT`); wired into the two local-player minion-play paths (drag-drop, tap-to-play-slot) only — spell plays and remote/AI plays don't get it. All new durations run through `BattleFx.scaled_duration(base, speed_scale)` (pure), keyed off the existing `_speed_scale` (1.0 normal / 0.45 fast-mode) — the co-op/PvP host-authoritative `_resolve_remote_attack()` intent-application path is untouched (flash-only), a deliberate v1 scope cut since it has no snapshot/refresh of its own at that call site.
- **Drag lift + reward count-up (TID-429):** dragging a hand card dims the source panel to `modulate.a = 0.45` (restored either by `NOTIFICATION_DRAG_END` in `BattleScene._notification()` — fires on drop success, drop failure, and cancel alike — or by the next `_refresh_all()`, since `CardViewBuilder.update_card_view()`/`_setup_empty_slot_panel()` unconditionally reset `visible`/`modulate`/`scale` on any reused panel now) and scales the ghost preview to `1.05`. `BattleResultUI._animate_count_up(lbl, target, fmt)` ticks a victory-screen coin/XP label from 0 to its value over 0.5s in up to 8 discrete steps (`BattleResultUI.count_up_steps()`, pure — always ends exactly on `target`), with a `ui_click` tick per step; used in both `show_victory()` and `show_victory_boss()`.
- "End Turn" button calls `GameState.end_turn()`; AI actions fire after a short delay for readability
- **Battle juice (GID-132 / TID-512, `scenes/battle/BattleJuice.gd`):** static helpers BattleFx calls. They live in their own file because BattleFx is near the 500-line lint limit.
  - `spawn_float_label(pos, text, color, amount)` → `pop_label`: back-ease pop to 1.3× then settle, a quad-eased 90 px rise with ±28 px drift, a late fade, and a size multiplier `label_scale(amount)` = 1 + 0.08 per point, capped at 1.8.
  - Every damage number also fires `sparks()`: a one-shot `CPUParticles2D` burst (10 + 2 per point, max 28, HDR white → orange → fade) on `_float_layer`.
  - `flash_from_snapshot` now flashes with `flash_color(heal)` and `punch()`es damaged heroes and cards: a squash/overshoot scale plus a small rotation wiggle, stronger at ≥ `BIG_HIT` (5).
  - `BattleFx.pop_new_board_cards()`, called at the end of `_refresh_all()`, diffs board instance ids per seat against the last call and runs `pop_in()` (60 % → 100 % back-ease, fade in) on newcomers. That covers player, AI and network plays. Cards that already got the hand→board travel ghost are skipped via `mark_board_seen()` (called from `_animate_card_travel`).
  - **Reduce Flashing:** keeps all motion, drops the sparks, and softens the flash toward white. Screen shake still obeys `screen_shake`. Settings are read from the `SaveManager` autoload directly, so unit tests work before SceneManager wires `save_manager`.
  - Tests: `test_battle_juice.gd`.
- **Action lock / anti multi-hit (claude/combat-bugs-ui-polish):** `BattleScene._action_busy` is true while a local attack lunge (`BattleInput._execute_attack`) or minion card-travel animates; `_can_local_act()` folds it in, so every tap/drag/End Turn is refused until the action settles. `_attempt_attack` also rechecks `attacker.can_attack()`. Previously the attack budget was spent only after the awaited lunge, so rapid taps stacked multiple attacks. `BattleFx.animate_attack` now tweens container-local `position` from a `lunge_home` meta (so an interrupted lunge can't park the panel mid-board) and `settle_panel()` restores it + `queue_sort()`s the HBox. Guarded by `tests/unit/test_battle_attack_lock.gd`.
- **Battle Effects panel:** `scenes/battle/BattleEffectsOverlay.gd` (BaseOverlay) lists every active modifier as `{title, desc, color}` rows: battlefield rule, day/night + active branch affinities, weather, gambit (+ reward multiplier), ambush, hero power (ready/used), passive skills, gear, companion. Entries are built by `BattleArena.collect_effect_entries()`; opened from the always-present side-panel "ⓘ Effects" button (`_add_effects_button`), or by tapping the battlefield info label / gambit badge (`make_opens_effects`). Stored in `_inspect_overlay`, so it blocks board input and Escape closes it. Hero power button and gambit badge also carry desktop `tooltip_text`. Tests: `test_battle_effects_overlay.gd`.
